...
class DataLoader:
    def __init__(self, data_path=None):
        # ========== 兼容参数+自动定位路径，彻底解决报错 ==========
        # 如果手动传了data_path，就用手动的；没传就自动定位到src/data文件夹
        if data_path is not None:
            self.data_path = data_path
        else:
            # 自动获取当前脚本（data_loader.py）所在的src文件夹路径
            current_script_dir = os.path.dirname(os.path.abspath(__file__))
            # 你的Excel文件都在src/data里，直接拼接这个路径
            self.data_path = os.path.join(current_script_dir, 'data')
    ...
    def load_round_data(self):
        ...
        # 提取轮次数字，按数字从小到大排序
        def get_round_num(file_path):
            filename = os.path.basename(file_path)
            # 提取"第X轮"里的数字X
            round_str = filename.replace('25-26赛季第', '').replace('轮.xlsx', '')
            try:
        ...
        # 强制排序
        round_files = sorted(round_files, key=get_round_num)
